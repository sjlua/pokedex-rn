...
import { Button, FlatList, Image, StyleSheet, useColorScheme, View } from "react-native";
...
export default function Collection() {
    ...
    async function fetchPokemon() {
        ...
        const unformattedResponse = await fetch(`https://pokeapi.co/api/v2/pokemon/?limit=${currentPage}&offset=${selectedRegion}`);
    ...
    return (
        ...
        <SafeAreaProvider>
            <SafeAreaView>
                <View style={[styles.questionRow, {backgroundColor: theme.background}]}>
                    <Text style={[styles.question, {color: theme.text}]}>{"Fly to another region?"}</Text>
                    <Button title={viewRegionPicker ? "Close" : "Search"} color={theme.iconColorFocused} onPress={() => showRegionPicker()}/>
                </View>

                { viewRegionPicker ? <View style={{backgroundColor: theme.background}}>
                    <Text style={[styles.smallQuestion, {color: theme.text}]}>{"By flying to another region, you can see all the Pokémon in that region, and beyond."}</Text>
                    <Picker
                ...
                <FlatList 
                    data={listPokemon}
                    numColumns={3}
                    style={{paddingHorizontal: 10, backgroundColor: theme.background}}
                    contentContainerStyle={{ gap: 10, margin: 10 }}
                    columnWrapperStyle={{ gap: 10, justifyContent: "center", width: "100%" }}
                    ...
                    <Link
                        ...
                        <View style={styles.cardContent}>
                            ...
                            {/* Name */}
                            <Text key={item.name} style={[styles.name, {color: theme.title}]}>
                            ...
                            {item.types.map((type) => (
                            <Text key={item.name + type.type.name} style={[styles.type, {color: theme.subtext}]}>{type.type.name}</Text>
                            ))}
                            </View>
